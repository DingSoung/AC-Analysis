/*fork from https://github.com/HezhouW/CRC16_CCITT */
...
// CRC16码表
unsigned int crc8table[256]={ /* CRC 字节余式表 */
...
unsigned int crc4table[16]={ /* CRC 半字节余式表 */
	0x0000,0x1021,0x2042,0x3063,0x4084,0x50a5,0x60c6,0x70e7,
	0x8108,0x9129,0xa14a,0xb16b,0xc18c,0xd1ad,0xe1ce,0xf1ef,
};
...
多项式采用CRC-CCITT 0x1021
*/
unsigned int crc_cal_by_byte(unsigned char* ptr, unsigned int len)
{
	unsigned short crc = 0;
	while(len-- != 0) {
		unsigned int high = (unsigned int)(crc/256); //取CRC高8位
		crc <<= 8;
		crc ^= crc8table[high^*ptr];
		ptr++;
	}
	return crc;
}
...
多项式采用CRC-CCITT  0x1021
*/
unsigned int crc_cal_by_halfbyte(unsigned char* ptr, unsigned int len)
{
	unsigned short crc = 0;
	while(len-- != 0) {
		unsigned char high = (unsigned char)(crc/4096); //暂存CRC的高4位
		crc <<= 4;
		crc ^= crc4table[high^(*ptr/16)];
		high = (unsigned char)(crc/4096);
		crc <<= 4;
		crc ^= crc4table[high^(*ptr&0x0f)];
		ptr++;
	}
	return crc;
}
